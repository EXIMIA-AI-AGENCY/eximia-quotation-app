...
export default function PackageCard({ package: pkg, selected, onSelect }: PackageCardProps) {
  ...
  return (
    <div
      className={`bg-card rounded-xl p-4 sm:p-6 cursor-pointer transition-all card-glow relative ${selected ? 'border-2 border-primary' : 'border border-border hover:border-primary/50'
        }`}
      onClick={onSelect}
    ...
    >
      ...
      {pkg.freeTrialDays && (
        <div className="absolute -top-2 sm:-top-3 right-2">
          <span className="bg-green-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs font-semibold">
      ...
      <div className="flex justify-center">
        <div
          className={`w-4 h-4 rounded-full border-2 ${selected
